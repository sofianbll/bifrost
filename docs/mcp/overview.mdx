...
Connect the app server as an ordinary MCP client with **Code Mode disabled** and grant its tools to the caller's Virtual Key. For Excalidraw, connect the host to Bifrost at `/mcp/excalidraw` (the client's endpoint slug). This route keeps callbacks such as `read_checkpoint` scoped to Excalidraw. The aggregate `/mcp` route exposes an app UI only when the caller has tools from a single upstream MCP client. With multiple clients, use each app's `/mcp/<slug>` route; aggregate tools remain available without app UI or app-only callbacks.
